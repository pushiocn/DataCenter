(function(angular, factory) {
	...
	angular.module('ngTable').run(['$templateCache',
		function($templateCache) {
			...
			$templateCache.put('ng-table/filters/time.html', '<input type="text" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="input-filter form-control"  date-picker date-fmt="yyyy-MM-dd H:mm:ss" min-date="" max-date=""  /> ');
			$templateCache.put('ng-table/filters/time-range.html', '<input id="d4311" class="am-radius" type="text" date-picker date-fmt="yyyy-MM-dd H:mm:ss" ng-model="item.effectiveDateS" min-date="" max-date="#F{$dp.$D(\'d4312\')||\'2020-10-01\'}" /><input id="d4312" class="am-radius" type="text" date-picker date-fmt="yyyy-MM-dd H:mm:ss" ng-model="item.effectiveDateE" min-date="#F{$dp.$D(\'d4311\')}" max-date=""/>');
		}
	]);
	return app;
}));